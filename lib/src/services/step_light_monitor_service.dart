...
class StepLightMonitorService extends ChangeNotifier {
  ...
  static const _syncStateLastCloudSyncMsKey =
      'step_light_sync_last_cloud_sync_ms';
  ...
  static const _stepSyncDebounce = Duration(seconds: 20);
  static const _stepSyncRetryInterval = Duration(minutes: 2);
  static const _minStepDeltaBeforeSync = 20;
  static const _maxSyncIntervalWithoutWrite = Duration(minutes: 2);
  ...
  bool _syncQueuedForce = false;
  bool _debouncedSyncForce = false;
  ...
  Future<void> _restoreSyncState() async {
    ...
    _lastCloudSyncMs = prefs.getInt(_syncStateLastCloudSyncMsKey) ?? 0;
  }
  ...
  Future<void> _handleAuthStateChanged(User? user) async {
    ...
    await _syncStepsToCloud(force: true);
  }
  ...
  Future<void> _saveSyncState() async {
    ...
    await prefs.setInt(_syncStateLastCloudSyncMsKey, _lastCloudSyncMs);
  }
  ...
  void _queueStepSync({bool force = false}) {
    if (_currentUid == null) return;

    if (force) {
      _debouncedSyncForce = true;
    }

    _stepSyncTimer?.cancel();
    _stepSyncTimer = Timer(_stepSyncDebounce, () {
      final shouldForce = _debouncedSyncForce;
      _debouncedSyncForce = false;
      _syncStepsToCloud(force: shouldForce);
    });
  }

  Future<void> _syncStepsToCloud({bool force = false}) async {
    ...
    final elapsedMs = nowMs - _lastCloudSyncMs;
    final canSyncByInterval =
        _lastCloudSyncMs == 0 ||
        elapsedMs >= _maxSyncIntervalWithoutWrite.inMilliseconds;
    final canSyncByDelta = delta >= _minStepDeltaBeforeSync;

    if (!force && !canSyncByDelta && !canSyncByInterval) {
      return;
    }

    if (_syncInProgress) {
      _syncQueued = true;
      _syncQueuedForce = _syncQueuedForce || force;
      return;
    }
    ...
    try {
      ...
      if (_syncQueued) {
        final queuedForce = _syncQueuedForce;
        _syncQueued = false;
        _syncQueuedForce = false;
        await _syncStepsToCloud(force: queuedForce);
      }
    }
  }
  ...
  Future<void> _startTracking() async {
    ...
    try {
      ...
      _syncRetryTimer ??= Timer.periodic(_stepSyncRetryInterval, (_) {
        if (_currentUid == null || _currentUid!.isEmpty) {
          return;
        }
        if (_steps > _lastSyncedSteps) {
          _syncStepsToCloud(force: true);
        }
      });
    } catch (e) {
